<?php
    // 6.Una compañía de seguros tiene contratados a N vendedores. 
    ...
    $n = (int) 3;
    $precioSeguros = (array) [ 50000, 80000, 120000 ];
    $base = (int) 550000;
    $porcentaje = (double) 0.10;
    $lista = (array) [];
    function ventas($id, $precioSeguros){
        $venta = (array) [];
        for ($i=0; $i < $id; $i++) { 
            array_push($venta, $precioSeguros[rand(0,count($precioSeguros)-1)]);
        }
        return $venta;
    }
    ...
    for ($i=0; $i < $n; $i++) { 
        array_push($lista, (array) [
            "VENTAS" => ventas(rand(1,3), $precioSeguros),
            "SUELDO" => $base
        ]);
